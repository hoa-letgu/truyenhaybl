    ...
    <div class="centernav">
        <div class="site-body">
            ...
            <div class="mb-4 manage-menu">
                <a href="{{ url("admin.chapter-add", ['m_id'=> $manga->id]) }}" class="btn mr-2"> <i
                        class="icofont-upload"></i> Thêm chương</a>
                <a href="{{ url("admin.manga-edit", ['m_id'=> $manga->id]) }}" class="btn"> <i
            ...
            <div class="content-manga-left">
                ...
                <div class="panel-manga-chapter " id="chapterlist">
                    <h2 class="manga-panel-title "><i class="icofont-flash"></i> {{ L::_("Latest Chapter Releases") }}
                    </h2>
                ...
                <div class="manga-tags ">
                    ...
                    <div class="tags_list">
                        ...
                        @foreach($tags as $tag)
                        <a href="{{ url('tag', ['t_slug' => $tag->slug, 't_id' => $tag->id]) }}" itemprop="keywords"
                            ...
                            <div class="rlbsx">
                                <div class="thumb">
                                    <a href="{{ url('manga', ['m_slug' => $manga->slug, 'm_id' => $manga->id]) }}"
                                        title="{{ $manga->name }}">
                                        <img data-src="{{ $manga->cover }}" src="{{ $manga->cover }}"
                                            alt="{{ $manga->name }}">
                                    </a>
                                </div>
                                <div class="bigor">
                                    ...
                                    <div class="list-chapter">
                                        @foreach(array_slice(get_manga_data('chapters', $manga->id, []),0,2) as
                                        $chapter)
                                        <div class="chapter-item">
                                            <span class="chapter">
                                                <a href="{{ url('chapter', ['m_slug' => $manga->slug, 'c_slug' => $chapter->slug, 'c_id' => $chapter->id ]) }}"
                                                    class="btn-link" title="{{ $manga->name }} {{ $chapter->name }}"> {{
                                                    $chapter->name }}@if($chapter->name_extend) - {{
                                                    $chapter->name_extend }} @endif</a>
                                            </span>
