<div id="header" class="header-reader">
    <div class="container">
        <div class="auto-div">
            <a href="{{ url('home') }}" id="logo" class="mr-0">
                <img src="/mangareader/images/logo.png" alt="logo hoitruyentranh">
                <div class="clearfix"></div>
            </a>
            <div class="hr-line"></div>
            <a title="{{ $manga->name }}" href="{{ url('manga', ['m_id' => $manga->id, 'm_slug' => $manga->slug]) }}"
                class="hr-manga">
                <h2 class="manga-name">{{ $manga->name }}</h2>
            </a>
            <div class="hr-navigation" style="display: none;">
                <div class="rt-item rt-read" style="display: none">
                    <button type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="btn">
                        <div class="d-block">{{ L::_('Chose server image') }}</div>
            ...
            <div class="float-right hr-right">
                <div class="hr-comment mr-2">
                    <a href="javascript:(0);" class="btn btn-sm hrr-btn">
                        ...
                        <span class="number">{{ (new \Models\Manga)->count_comment($manga->id) }}</span>
                        <span class="hrr-name">Comments</span>
                    </a>
                    <div class="clearfix"></div>
                </div>
                <div class="hr-setting mr-2" style="display: none">
                    <a class="btn btn-sm hrr-btn"><i class="fas fa-cog"></i><span class="hrr-name">{{ L::_('Settings')
                            }}</span></a>
                    <div class="clearfix"></div>
                </div>
                <div class="hr-info mr-2">
                    <a title="{{ $manga->name }}"
                        href="{{ url('manga', ['m_id' => $manga->id, 'm_slug' => $manga->slug]) }}"
                        class="btn btn-sm hrr-btn"><i class="fas fa-info"></i><span class="hrr-name">{{ L::_('Manga
                            Detail') }}</span></a>
                    <div class="clearfix"></div>
                </div>
