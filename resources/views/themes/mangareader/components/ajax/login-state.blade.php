...
<div id="user-slot">
    <div class="header_right-user user-logged">
        <a class="btn-user btn btn-login" data-target="#modal-auth" data-toggle="modal"><i class="fas fa-user-circle mr-2"></i>{{ L::_('Member') }}</a>
    </div>
</div>
@else
    ...
    <div class="header_right-user logged">
        <div class="dropdown">
            <div aria-expanded="false" class="btn-avatar" data-toggle="dropdown">
                <img alt="{{ userget()->name }}" src="{{ userget()->avatar_url }}">
            </div>
            <div class="dropdown-menu dropdown-menu-model" id="user_menu">
                <a class="dropdown-item" href="/user/payment">
                    <i style="color: #ffd702" class="fas fa-coins mr-2"></i>Token: {{ userget()->coin }}
                </a>
                ...
                <a class="dropdown-item" href="{{ url('admin') }}">
                    <i class="fas fa-tachometer-alt mr-2"></i>{{ L::_('Dashboard') }}
                </a>
                @endif
                ...
                <a class="dropdown-item" href="/user/notification">
                    <i class="fas fa-bell mr-2"></i>{{ L::_('Notifications') }}
                </a>
                <a class="dropdown-item di-bottom" href="/logout">{{ L::_('Logout') }}<i class="fas fa-arrow-right ml-2 mr-1"></i></a>
            </div>
        </div>
    </div>
</div>
