...
function getimgEl(imgUrl, onload = null) {
    var imgEl = new Image();
    var totalError = 0;
    ...
    // No referrer list
    var noReferrerListKey = ['ggpht', 'kakaocdn', 'googleusercontent'];
    if (noReferrerListKey.some((key) => imgUrl.includes(key) && !imgUrl.includes('proxy'))) {
    ...
    imgEl.onerror = function () {
        if (listServer[totalError] !== undefined) {
            imgEl.src = listServer[totalError++] + imgUrl;
            return;
        }
...
function imgReverser(imgUrl) {
    return new Promise((myResolve) => {
        var imgEl = getimgEl(imgUrl, function () {
            let canvas = imageLoader.decrypt(imgEl, keys, factor);

            if (typeof userID !== 'undefined' && userID !== null) {
                let ctx = canvas.getContext("2d");

                var textWatermark = userID;
                if (textWatermark.length < 10) {
                    textWatermark = textWatermark.padStart(10, '0');
                }

                // font size by image width
                if (canvas.width < 500) {
                    var fontSize = "10px";
                ...
                ctx.fillText(textWatermark, canvas.width / 1.4, canvas.height / 1.2);
